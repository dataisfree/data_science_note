...
df = pd.read_csv('czw_installment_ShoppingMall_test_clean_trainANDtest_clean.csv')
...
# 重点关注负向指标时，改变report的值
# report_bad = result_stacking_proba[:, 1] >= 0.8
# report_bad_new = []
# for i in report_bad:
#     if i == True:
#         report_bad_new.append(0)
#     else:
#         report_bad_new.append(1)
#
# print(classification_report(y_test, report_bad_new, target_names=['0', '1']))

# 重点关注正向指标时，改变report的值
report_right= result_stacking_proba[:, 0] >= 0.8
report_right_new = []
for i in report_right:
    if i == True:
        report_right_new.append(0)
    else:
        report_right_new.append(1)

print(classification_report(y_test, report_right_new, target_names=['0', '1']))
